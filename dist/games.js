...
(() => {
  ...
  const games = entries.map(([id,title,subtitle,category,icon,color,component,engineType]) => ({ id,title,subtitle,category,icon,thumbnail:icon,color,component,engineType,status:['neon-race','block-grid','beat-dash','neon-hook','penalty-duel','core-drop','void'].includes(id)?'available':'coming-soon',featured:id==='neon-race' }));
