 ...
 *
 * Prices are intentionally absent from the built-in draft catalog. An owner
 * must configure TELEPLAY_PAYMENT_PACKAGES in Edge Function secrets before an
 * invoice can be created. This keeps the repository free of real prices and
 * makes every amount authoritative on the backend.
 */
...
const normalize = entry => {
  ...
  const id = safeText(value.id, 64), name = safeText(value.name, 32), description = safeText(value.description || value.name, 255);
  const gemsAmount = positiveInteger(value.gemsAmount), amount = positiveInteger(value.amount ?? value.starsPrice);
  const bonus = nonNegativeInteger(value.bonus);
  const active = value.active === undefined ? value.enabled !== false : value.active === true;
  const currency = safeText(value.currency || 'XTR', 3).toUpperCase();
...
export const paymentPackages = env => {
  const raw = String(env?.TELEPLAY_PAYMENT_PACKAGES || '').trim();
  if (!raw) return [...DRAFT_PACKAGES];
  try {
    const parsed = JSON.parse(raw), list = Array.isArray(parsed) ? parsed : Object.values(parsed || {});
